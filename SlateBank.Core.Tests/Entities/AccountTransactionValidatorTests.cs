...
namespace SlateBank.Core.Tests.Entities
{
    public class AccountTransactionValidatorTests
    {
        ...
        {
            var at = new AccountTransaction {Amount = 0m};
        ...
        [Fact]
        public void Should_Pass_For_Found_Active_AccountNumber()
        {
            ...
            dataStoreMock.Setup(ds => ds.IsAccountActive(accountNumber)).Returns(true);
        ...
        [Fact]
        public void Should_Fail_For_Found_Inactive_AccountNumber()
        {
            // do some setup for constructor DI:
            var dataStoreMock = new Mock<IDataStore>();
            const string accountNumber = "100";
            dataStoreMock.Setup(ds => ds.AccountNumberExists(accountNumber)).Returns(true);
            dataStoreMock.Setup(ds => ds.IsAccountActive(accountNumber)).Returns(false);
